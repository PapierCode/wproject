...
function pc_xxx_admin_edit_page_content_from_one_time( $metabox_select_content_from, $post ) {
	...
	if( is_object( $page_with_products ) && $page_with_products->ID != $post->ID ) {
		...
		unset( $metabox_select_content_from[XXX_POST_SLUG] );
...
add_action( 'manage_'.XXX_POST_SLUG.'_posts_columns', 'pc_admin_list_column_img', 10, 2);
add_action( 'manage_'.XXX_POST_SLUG.'_posts_custom_column', 'pc_admin_list_column_img_content', 10, 2);
