...
add_action( 'woocommerce_before_shop_loop_item', 'pc_woo_article_tag_start', 10 );
...
add_action( 'woocommerce_after_shop_loop_item_title', 'pc_woo_product_resum_wrap_cart_start', 5 );
...
add_action( 'woocommerce_after_shop_loop_item', 'pc_woo_product_resum_wrap_cart_end', 20 );
// article end (content-product.php)
add_action( 'woocommerce_after_shop_loop_item', 'pc_woo_article_tag_end', 30 );
...
function pc_woo_product_resum_display_content( $custom_product, $hn = 2 ) {
	...
	echo '<h'.$hn.' class="st-title">'.pc_woo_resum_get_link_tag_start( 'product', 'st-title-link', $product_link, $product_title ).$product_title.'</a></h'.$hn.'>';	

	if ( '' != $product_desc ) {
		echo '<p class="st-desc">'.$product_desc.'...</p>';
	}

	echo pc_woo_resum_get_link_tag_start( 'product', 'st-read-more button', $product_link, $product_title ).'<span class="st-read-more-ico">'.pc_svg('more-16').'</span> <span class="st-read-more-txt">En savoir plus</span><span class="visually-hidden"> sur le produit '.$product_title.'</span></a>';
